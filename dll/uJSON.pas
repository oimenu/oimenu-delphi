...
begin
  ...
  if (o = nil) then
  begin
    raise NoSuchElementException.create('TJSONObject[' + quote(key) + '] not found.');
  end;
  result := o;
end;
...
begin
  ...
  else if (o.equals(_Boolean._TRUE) or ((o is _String) and (_String(o)).equalsIgnoreCase('true'))) then
  begin
    result := true;
    exit;
  end;
  raise ClassCastException.create('TJSONObject[' + quote(key) + '] is not a Boolean.');
end;
...
begin
  ...
  raise NumberFormatException.create('TJSONObject[' + quote(key) + '] is not a number.');
end;
...
begin
  ...
  begin
    raise NoSuchElementException.create('TJSONObject[' + quote(key) + '] is not a TJSONArray.');
  end;
end;
...
begin
  ...
  begin
    raise NoSuchElementException.create('TJSONObject[' + quote(key) + '] is not a TJSONObject.');
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      sb := sb + quote(o);
      sb := sb + ': ';
      sb := sb + valueToString(TZAbstractObject(myHashMap.Objects[myHashMap.IndexOf(o)]), indentFactor, indent);
    end
    else
    begin
      ...
      begin
        ...
        sb := sb + quote(o);
        sb := sb + ': ';
        sb := sb + valueToString(TZAbstractObject(myHashMap.Objects[myHashMap.IndexOf(o)]), indentFactor, newindent);
      end;
      if (System.length(sb) > 1) then
      begin
        sb := sb + #10;
...
begin
  ...
  if ((value is _Boolean) or (value is TJSONObject) or (value is TJSONArray)) then
  begin
...
(**
     ...
     *  with <code>}</code>&nbsp;<small>(right brace)</small>.
     *)
class function TJSONObject.valueToString(value: TZAbstractObject; indentFactor, indent: integer): string;
begin
...
begin
  ...
  raise ClassCastException.create('TJSONArray[' + intToStr(index) + '] not a Boolean.');
end;
...
begin
  ...
  raise NumberFormatException.create('TJSONObject[' + intToStr(index) + '] is not a number.');
end;
...
begin
  ...
  raise NoSuchElementException.create('TJSONArray[' + intToStr(index) + '] is not a TJSONArray.');
end;
...
begin
  ...
  begin
    ...
    raise NoSuchElementException.create('TJSONArray[' + intToStr(index) + '] is not a TJSONObject is ' + s);
  end;
end;
...
begin
  ...
  begin
    if (myHashMap.Objects[0] <> CONST_FALSE) and (myHashMap.Objects[0] <> CONST_TRUE) and (myHashMap.Objects[0] <> CNULL) then
    begin
